...
import { getRandomAdvocateResponse } from '../data/devilsAdvocate';
...
export const DevilsAdvocateModal = ({ lang = 'en', onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setResponse(getRandomAdvocateResponse(lang));
  };
        ...
        {!response ? (
          <form onSubmit={handleSubmit}>
            <p className="font-serif italic text-[17px] leading-relaxed text-ink mb-4">
              {lang === 'es' ? '¿Quiere discutir con el Diablo?' : 'Do you want to argue with the Devil?'}
            </p>
            <label htmlFor="advocate-opinion" className="sr-only">
              {lang === 'es' ? 'Su opinión' : 'Your opinion'}
            </label>
            <textarea
              id="advocate-opinion"
              value={opinion}
              onChange={(e) => setOpinion(e.target.value)}
              placeholder={lang === 'es' ? 'Escriba su objeción…' : 'Write your objection…'}
              rows={3}
              autoFocus
